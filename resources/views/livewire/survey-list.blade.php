<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8">
                        ...
                        <div class="flex items-center">
                            ...
                            <div class="ml-5 w-0 flex-1">
                                <dl>
                                    ...
                                    <dd class="text-lg font-semibold text-gray-900">{{ $totalSurveys }}</dd>
                                </dl>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="flex items-center">
                            ...
                            <div class="ml-5 w-0 flex-1">
                                <dl>
                                    ...
                                    <dd class="text-lg font-semibold text-gray-900">{{ collect($surveys)->where('status', 'active')->count() }}</dd>
                                </dl>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="flex items-center">
                            ...
                            <div class="ml-5 w-0 flex-1">
                                <dl>
                                    ...
                                    <dd class="text-lg font-semibold text-gray-900">{{ collect($surveys)->where('status', 'draft')->count() }}</dd>
                                </dl>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="flex items-center">
                            ...
                            <div class="ml-5 w-0 flex-1">
                                <dl>
                                    ...
                                    <dd class="text-lg font-semibold text-gray-900">{{ collect($surveys)->where('status', 'completed')->count() }}</dd>
                                </dl>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="p-6">
                            <div class="flex items-center justify-between mb-4">
                                <div class="flex items-center space-x-2">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold
                                        ...
                                        <div class="w-2 h-2 rounded-full mr-1.5
                                            ...
                                            {{ $survey['status'] === 'draft' ? 'bg-gray-500' : '' }}
                                            {{ $survey['status'] === 'paused' ? 'bg-yellow-500' : '' }}
                                            {{ $survey['status'] === 'completed' ? 'bg-blue-500' : '' }}
                                            {{ $survey['status'] === 'archived' ? 'bg-red-500' : '' }}"></div>
                                ...
                                <div class="flex space-x-1">
                                    <button wire:click="editSurvey('{{ $survey['uuid'] }}')"
                                        class="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                        </svg>
                                    </button>
                                    <button wire:click="confirmDelete('{{ $survey['uuid'] }}')"
                            ...
                            <h3 class="text-xl font-bold text-gray-900 mb-3">{{ $survey['name'] }}</h3>
                            <p class="text-sm text-gray-600 mb-4 line-clamp-2">{{ \Illuminate\Support\Str::limit($survey['description'] ?? 'No description provided', 120) }}</p>
    ...
    @if($showEditModal && $selectedSurvey)
    ...
    @if($showDeleteModal && $selectedSurvey)
    <div class="fixed inset-0 bg-gray-600 bg-opacity-75 overflow-y-auto h-full w-full z-50 flex items-center justify-center" wire:click="closeModals">
        <div class="relative bg-white rounded-lg shadow-xl max-w-sm w-full mx-4" wire:click.stop>
            <div class="px-6 py-4">
                ...
                <p class="text-sm text-gray-500 text-center mb-6">
                    Are you sure you want to delete "<span class="font-medium text-gray-900">{{ $selectedSurvey->name }}</span>"? This action cannot be undone.
                </p>
